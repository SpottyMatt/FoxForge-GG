...
- **Footer** — legal disclaimer and copyright live in Settings → Legal (`LEGAL_DISCLAIMER` and attribution from `src/ui/brand.ts`); patch version appears only under Settings → Updates. Nothing from these sections is rendered in `App.tsx`.
