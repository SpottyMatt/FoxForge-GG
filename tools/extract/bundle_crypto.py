"""Pokémon UNITE asset-bundle decryption.

Ported from kwsch/UntieUnite (UntieUnite.Core/AssetCrypto.cs + Dumper.cs).
UNITE bundles are standard UnityFS containers whose top metadata + compressed
block-info are obfuscated with a custom XOR:

    plain[i] = ((cipher[i] ^ ~key[i % len]) + 0x49) & 0xFF

Two static keys: a 16-byte MetaDataKey over the size fields, and a 1013-byte
BlockInfoKey over the compressed block info. Encryption is flagged in the
bundle's `flags` word. UnityPy does NOT implement this scheme (it mistakes the
flag for UnityCN AES), so we decrypt in-place first, clear the flag, and hand a
clean UnityFS bundle to UnityPy.
"""
...
META_DATA_KEY = bytes([0x12,0x16,0x60,0xCA,0x64,0x0C,0xFD,0xCE,0xE1,0x6D,0x74,0x3C,0x0D,0x1A,0x99,0x6A])

# 1013-byte block-info key (parsed verbatim from AssetCrypto.cs).
BLOCK_INFO_KEY = bytes([
...
assert len(META_DATA_KEY) == 16 and len(BLOCK_INFO_KEY) == 1013

# Either bit has been observed marking an encrypted UNITE bundle (0x200 in the
# UntieUnite-era builds; 0x400 in 1.23.x format-7 bundles).
ENC_FLAG_BITS = 0x200 | 0x400


def _xor_decrypt(buf: bytearray, offset: int, size: int, key: bytes) -> None:
    klen = len(key)
    for i in range(size):
        buf[offset + i] = ((buf[offset + i] ^ (~key[i % klen] & 0xFF)) + 0x49) & 0xFF


def _header_offset(buf) -> int:
    sig_len = buf.index(0, 0)                  # after "UnityFS"
    uver_end = buf.index(0, sig_len + 1 + 4)   # after "5.x.x"
    urev_end = buf.index(0, uver_end + 1)      # after "2019.4.15f1"
    return urev_end + 1                         # start of size fields


def is_encrypted(raw: bytes) -> bool:
    if raw[:7] != b"UnityFS":
        return False
    offset = _header_offset(raw)
    flags = struct.unpack_from(">I", raw, offset + 0x10)[0]
    return (flags & ENC_FLAG_BITS) != 0


def decrypt_bundle(raw: bytes) -> bytes:
    """Return a decrypted, UnityPy-readable copy of a UNITE *.bundle.
    If the bundle is not UNITE-encrypted, returns it unchanged."""
    if raw[:7] != b"UnityFS":
        return raw
    buf = bytearray(raw)
    offset = _header_offset(buf)

    flags = struct.unpack_from(">I", buf, offset + 0x10)[0]
    enc_bit = flags & ENC_FLAG_BITS
    if enc_bit == 0:
        return raw  # not encrypted

    # 1. size fields (16 bytes): int64 size + int32 compBlockInfo + int32 uncomp
    _xor_decrypt(buf, offset, len(META_DATA_KEY), META_DATA_KEY)

    # 2. clear the encryption flag so UnityPy sees a normal UnityFS bundle
    struct.pack_into(">I", buf, offset + 0x10, flags & ~enc_bit)

    # 3. compressed block info (compBlockInfoSize bytes) right after the header
    comp_block_info_size = struct.unpack_from(">i", buf, offset + 0x8)[0]
    _xor_decrypt(buf, offset + 0x14, comp_block_info_size, BLOCK_INFO_KEY)
