...
// --- Held items: max-level (30) flats ---
const floatStone = bundle.heldItems.find((i) => i.id === "float-stone")!;
check("Float Stone +28 Attack", floatStone.statsByGrade[30].attack, 28);
check("Float Stone +175 MoveSpeed", floatStone.statsByGrade[30].moveSpeed, 175);
const muscle = bundle.heldItems.find((i) => i.id === "muscle-band")!;
check("Muscle Band +17.5 Attack", muscle.statsByGrade[30].attack, 17.5);
check("Muscle Band +8.75% AtkSpeed", muscle.statsByGrade[30].attackSpeed, 0.0875);
const wise = bundle.heldItems.find((i) => i.id === "wise-glasses")!;
check("Wise Glasses +44 SpAtk", wise.statsByGrade[30].spAttack, 44);
